...
local registBackGround,registText,registNameBox,registNameText,registEmailBox,registEmailText,registPasswordBox,registPasswordText,registPasswordConfirmationText,registPasswordConfirmationBox,registWebsiteBox,registWebsiteText,registNoteBox,registNoteText,registSendButton, registLine
local flash
...
function scene:create( event )
    ...
    --line
    registLine = display.newImageRect( "imgs/line.png", 500, 70)
    registLine.x = screenW-100
    registLine.y = screenH-460

    registText = display.newText("アカウント登録",screenW/3, screenH/15, nil, 30)
    registText:setFillColor( 1,1,1 )

    registNameText = display.newText("名前 :",85,100,nil,15)
    registNameText:setFillColor( 0, 0, 0 )
    registNameBox  = native.newTextField(screenW/2+50,100,200,30)

    registEmailText = display.newText("メールアドレス :",58,140,nil,15)
    registEmailText:setFillColor( 0, 0, 0 )
    registEmailBox  = native.newTextField(screenW/2+50,140,200,30)

    registPasswordText = display.newText("パスワード :",70,180,nil,15)
    registPasswordText:setFillColor( 0, 0, 0 )
    registPasswordBox  = native.newTextField(screenW/2+50,180,200,30)
    registPasswordBox.isSecure = true

    registPasswordConfirmationText = display.newText("パスワード(再) :",58,220,nil,15)
    registPasswordConfirmationText:setFillColor( 0, 0, 0 )
    registPasswordConfirmationBox  = native.newTextField(screenW/2+50,220,200,30)
    registPasswordConfirmationBox.isSecure = true

    registWebsiteText = display.newText("WEBサイト :",65,260)
    registWebsiteText:setFillColor( 0, 0, 0 )
    registWebsiteBox  = native.newTextField(screenW/2+50,260,200,30)

    registNoteText = display.newText("備考 :",85,300)
    registNoteText:setFillColor( 0, 0, 0 )
    registNoteBox  = native.newTextField(screenW/2+50,300,200,30)
